...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const tasks = await askOpenAITasks(
      buildSystemTasksPrompt(language),
      buildTasksUserMessage(topic, partyDetails, selectedTiles, language),
      buildTasksUserMessageCompact(topic, partyDetails, selectedTiles, language)
    )
